...
public class HashClient implements Closeable {
    ...
    public HashClient(ServiceCachingPolicy cachingPolicy, File configYAMLFile) throws Exception {
        ...
        HashServiceConfiguration configuration = getConfiguration(HashServiceConfiguration.class, configYAMLFile);
        ...
        _calculatorPool = ServicePoolBuilder.create(HashServiceClient.class)
                ...
                .withHostDiscovery(new ZooKeeperHostDiscovery(_curatorFramework, serviceFactory.getServiceName()))
                .withCachingPolicy(cachingPolicy)
                .build();

        HealthChecks.register(ContainsHealthyEndPointCheck.forPool(_calculatorPool, HashService.getServiceName()));
        _actualCallRate = 1;
    ...
    public TransportData call(final String function, final String data) {
        TimerContext clock = _executionTimer.time();
        try {
